...
FIXTURE_TEST(diverse_replica_sets, partition_allocator_fixture) {
    // This tests that all possible replica sets are chosen, rather than some fixed
    // subset (e.g., if the allocator uses a repeating sequential pattern for allocating
    // replica sets, most sets won't be chosen).
    ...
    // for the 6 nodes, r=3 case, all replica sets will be chosen with probability about
    // 1 - 1e-21 (i.e., with an astronomically high chance) after 1,000 samples.
...
FIXTURE_TEST(rack_aware_assignment_1, partition_allocator_fixture) {
    ...
    BOOST_REQUIRE(nodes.contains(model::node_id(0)) || nodes.contains(model::node_id(1))); // rack-a
    BOOST_REQUIRE(nodes.contains(model::node_id(2)) || nodes.contains(model::node_id(3))); // rack-b
    BOOST_REQUIRE(nodes.contains(model::node_id(4)) || nodes.contains(model::node_id(5))); // rack-c
}
