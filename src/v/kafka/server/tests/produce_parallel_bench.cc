...
struct produce_partition_fixture : redpanda_thread_fixture {
    ...
    ss::future<> run_test(test_args args) {
        return smp::invoke_on_all([args, this] { return inner_test(args); });
    };

    ss::future<> inner_test(test_args args);
};

ss::future<> produce_partition_fixture::inner_test(test_args args) {
    // fmt::print("SMP count: {}\n", ss::smp::count);
    ...
    for (size_t i = 0; i < args.num_records; ++i) {
        builder.add_raw_kv(iobuf{}, rand_iobuf(args.data_size));
    }

    auto original_batch = std::move(builder).build();
    perf_tests::start_measuring_time();

    for (size_t i = 0; i < args.request_count; ++i) {
        model::topic_partition tp = model::topic_partition(
          t, model::partition_id(0));
        auto batch = original_batch.copy();
        ...
        vassert(!fut.failed(), "dispatch failed: {}", fut.get_exception());
        // if (fut.failed()) {
        //     vlog(
        //       plog.error,
...
PERF_TEST_C(produce_partition_fixture, 1_KiB_produced_1) {
    co_return co_await this->run_test(
      test_args{.data_size = 1024, .num_records = 100, .request_count = 1});
}

PERF_TEST_C(produce_partition_fixture, 1_KiB_produced_10) {
    co_return co_await this->run_test(
      test_args{.data_size = 1024, .num_records = 100, .request_count = 10});
}

PERF_TEST_C(produce_partition_fixture, 1_KiB_produced_100) {
    co_return co_await this->run_test(
      test_args{.data_size = 1024, .num_records = 100, .request_count = 100});
}

// PERF_TEST_C(produce_partition_fixture, 1_KiB_produced_10) {
//     co_return co_await this->run_test(
//       test_args{.data_size = 1024, .num_records = 10, .request_count = 3});
// }

// PERF_TEST_C(produce_partition_fixture, 1_KiB_produced_1) {
//     co_return co_await this->run_test(
//       test_args{.data_size = 1024, .num_records = 1, .request_count = 3});
// }
